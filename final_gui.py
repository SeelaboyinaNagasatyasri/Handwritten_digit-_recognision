...
result_label = Label(root, text="Draw a digit and click Recognize", font=("Arial", 14))
result_label.grid(row=1, column=0, columnspan=2, pady=5)

def clear_widget():
    cv.delete('all')  
    result_label.config(text="Canvas cleared. Draw again!")
...
def Recognize_Digit():
    ...
    for cnt in contours:
        ...
        # also update Tkinter label
        result_label.config(text=f"Prediction: {final_pred} ({confidence}%)")
